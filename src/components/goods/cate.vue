<template>
    <div>
        ...
        <el-card class="addkinds">
            ...
            <!-- 表格区域 -->
            <zk-table :data="cateList" :columns="columns" :expand-type="false" :selection-type="false"
                :show-index="true" index-text="#" border :show-row-hover="false" class="zktable">
                <!-- 是否有效 -->
                <template slot="isok" slot-scope="res">
                    <i class="el-icon-success" v-if="res.row.cat_deleted === false" style="color:rgb(103, 194, 58)"></i>
                ...
                <template slot="order" slot-scope="res">
                    ...
                    <el-tag type="success" v-else-if="res.row.cat_level === 1">二级</el-tag>
        ...
        <!-- 添加分类的对话框 -->
        <el-dialog title="添加用户" :visible.sync="editCatedialogVisible" width="50%" @close="addCateClosed">
            <!-- 内容主题区域 -->
            <el-form :model="addCateForm" :rules="addCaterules" ref="addCateruleForm" label-width="100px"
                ...
                <el-form-item label="父级分类">
                    <!-- 级联选择器 -->
                    <!-- options 用来存储数据源 -->
                    <el-cascader clearable v-model="selectedKeys" :options="parentCateList" :props="myProps"
                        @change="parentCateChange" ref="cateLevel">
                    </el-cascader>
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="addCate('addCateruleForm')">确 定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
    ...
    methods: {
        ...
        addCate: function (formName) {
            ...
            this.$refs[formName].validate(async (valid) => {
                if (valid) {
                    // 发起请求
                    const { data } = await this.axios.post(`categories`, this.addCateForm);
                    // console.log(data);
